...
docker exec -- ${OPENLDAP} ldapadd -c -f /tmp/secauthority.ldif -H "ldaps://localhost:636" -D "cn=root,secAuthority=Default" -w "Passw0rd" > /tmp/isva-restore.log 2>&1
docker cp ${TMPDIR}/ibmcom.ldif ${OPENLDAP}:/tmp/ibmcom.ldif
docker exec -- ${OPENLDAP} ldapadd -c -f /tmp/ibmcom.ldif -H "ldaps://localhost:636" -D "cn=root,secAuthority=Default" -w "Passw0rd" >> /tmp/isva-restore.log 2>&1
...
docker exec -i -- ${POSTGRESQL} /usr/local/bin/psql isva < ${TMPDIR}/isva.db >> /tmp/isva-restore.log 2>&1

# Get docker container ID for isvaconfig container
ISVACONFIG="$(docker-compose -f ${YAML} ps -q isvaconfig)"

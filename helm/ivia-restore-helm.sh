...
OPENLDAP="$(kubectl get --no-headers=true pods -l app=iamlab-iviaopenldap -o custom-columns=:metadata.name)"
...
POSTGRESQL="$(kubectl get --no-headers=true pods -l app=iamlab-iviapostgresql -o custom-columns=:metadata.name)"

# Restore DB
echo "Loading DB Data..."
kubectl exec ${POSTGRESQL} -i -- /usr/local/bin/psql ivia < ${TMPDIR}/ivia.db >> /tmp/ivia-restore.log 2>&1

# Get docker container ID for iviaconfig container
ISVACONFIG="$(kubectl get --no-headers=true pods -l app=iamlab-iviaconfig -o custom-columns=:metadata.name)"

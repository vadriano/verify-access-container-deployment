...
docker run -t -d --restart always -v isvaconfig:/var/shared --hostname isvawrprp1 --name isvawrprp1 -e CONTAINER_TIMEZONE=Europe/London -p ${MY_WEB1_IP}:443:9443 -e SERVICE=webseal -e INSTANCE=rp1 -e AUTO_RELOAD_FREQUENCY=5 --network isva ibmcom/verify-access-wrp:${ISVA_VERSION}
